...
#            c) Fossil voyage  (HSFO, LFO, MGO): ascending WtW, **each capped at 50% per fuel** with partial on last to close pool.
# RFNBO reward:
...
def scoped_energies_extra_eu(energies_fuel_voyage: Dict[str, float],
    ...
    Extra-EU (requested variant):
      ...
      • Re-fill the pool (without changing its total) by WtW priority:
          ...
          3) Fossil voyage by WtW (HSFO/LFO/MGO), **each capped at 50% of its voyage energy**, partial on last if needed
    ...
    # 3) Fossil voyage by ascending WtW — take 50% of each fuel, with partial on last to close pool
    for f in foss_sorted:
        half_voy_fuel = 0.5 * g(energies_fuel_voyage, f)  # per-fuel 50% cap
...
with st.sidebar:
    ...
    # Preview factor for €/tCO2e ↔ €/VLSFO-eq t linking (based on current mix, with 2025 RFNBO reward in denominator)
    if "Extra-EU" in voyage_type:
    ...
    # Credits — PRIORITY: €/tCO2e (user edits) → derive €/VLSFO-eq t (read-only)
    ...
    # Penalties — PRIORITY: €/VLSFO-eq t (user edits) → derive €/tCO2e (read-only)
    ...
    penalty_price_eur_per_tco2e = (penalty_price_eur_per_vlsfo_t / factor_vlsfo_per_tco2e) if factor_vlsfo_per_tco2e > 0 else 0.0
    st.text_input("Penalty price €/tCO₂e (derived)", value=us2(penalty_price_eur_per_tco2e), disabled=True)
    ...
    # Pooling input + start year (start-year just below input)
